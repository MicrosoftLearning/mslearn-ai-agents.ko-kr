...
이 작업에서는 원격 MCP 서버에 연결하고, AI 에이전트를 준비하고, 사용자 프롬프트를 실행합니다.
...
1. **에이전트 MCP 도구 초기화** 주석 아래에 다음 코드를 추가합니다.
   ...
   mcp_tool = McpTool(
    ...
    이 코드는 AI 에이전트가 기존 github 리포지토리에 대한 문서 쿼리를 연결하고 지원할 수 있도록 하는 원격 Git MCP 서버에 연결합니다. 
...
1. **MCP 도구를 사용하여 스레드에서 실행되는 에이전트 만들기 및 처리** 주석을 찾아 다음 코드를 추가합니다.
   ...
   print(f"Created run, ID: {run.id}")
    ...
    AI 에이전트는 연결된 MCP 도구를 자동으로 호출하여 즉각적인 요청을 처리합니다. 이 프로세스를 설명하기 위해 **실행 단계 및 도구 호출 표시** 주석 아래에 제공된 코드는 MCP 서버에서 호출된 모든 도구를 출력합니다.
...
1. 로그인한 후 다음 명령을 입력하여 애플리케이션을 실행합니다.
   ...
   python client.py
    ...
    에이전트가 MCP 도구 `fetch_azure_rest_api_docs`를 자동으로 호출하여 요청을 이행할 수 있었음을 확인하세요.
