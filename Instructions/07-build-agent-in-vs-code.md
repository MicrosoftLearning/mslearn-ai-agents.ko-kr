...
lab:
  ...
  description: Microsoft Foundry VS Code 확장을 사용하여 AI 에이전트를 만듭니다.
---

# VS Code 확장을 사용하여 AI 에이전트 개발

이 연습에서는 Microsoft Foundry VS Code 확장을 사용하여 MCP(모델 컨텍스트 프로토콜) 서버 도구를 통해 외부 데이터 원본 및 API에 액세스할 수 있는 에이전트를 만듭니다. 에이전트는 MCP 도구를 통해 최신 정보를 검색하고 다양한 서비스와 상호 작용할 수 있습니다.
...
## Microsoft Foundry VS Code 확장 설치
...
1. 검색 창에서 **Microsoft Foundry**를 입력하고 Enter 키를 누릅니다.

1. Microsoft에서 **Microsoft Foundry** 확장을 선택하고 **설치**를 클릭합니다.
...
1. VS Code 사이드바에서 **Microsoft Foundry** 확장 아이콘을 선택합니다.

1. 리소스 보기에서 **Azure에 로그인...** 을 선택하고 인증 프롬프트를 따릅니다.

   > **참고**: 이미 로그인한 경우에는 이 옵션이 표시되지 않습니다.

1. Foundry 확장 보기에서 **리소스** 옆에 있는 **+**(더하기) 아이콘을 선택하여 새 Foundry 프로젝트를 만듭니다.

1. 드롭다운에서 해당 Azure 구독을 선택합니다.
...
1. 텍스트 상자에 Foundry 프로젝트의 이름(예: "ai-agents-project")을 입력하고 Enter 키를 누릅니다.
...
1. "프로젝트가 배포되었습니다." 팝업이 나타나면 **모델 배포** 단추를 선택합니다. 그러면 모델 카탈로그가 열립니다.
   ...
   > **팁**: 리소스 섹션에서 **모델** 옆에 있는 **+** 아이콘을 선택하거나 **F1** 키를 누르고 명령 **Microsoft Foundry: 모델 카탈로그 열기**를 실행하여 모델 카탈로그에 액세스할 수도 있습니다.

1. 모델 카탈로그에서 **gpt-4** 모델을 찾습니다(검색 창을 사용하여 신속하게 찾을 수 있음).

    ![Foundry VS Code 확장의 모델 카탈로그 스크린샷](Media/vs-code-model.png)

1. gpt-4 모델 옆에 있는 **배포**를 선택합니다.

1. 배포 설정을 구성합니다.
   - **배포 이름**: "gpt-4-deployment"와 같은 이름을 입력합니다.
   - **배포 유형**: **글로벌 표준**(또는 글로벌 표준을 사용할 수 없는 경우 **표준**)을 선택합니다.
   - **모델 버전**: 기본값 유지
   - **분당 토큰 수**: 기본값 유지

1. 왼쪽 아래 모서리에 있는 **Microsoft Foundry에서 배포**를 선택합니다.
...
이제 비주얼 디자이너 인터페이스를 사용하여 AI 에이전트를 만듭니다. 코드를 작성하는 대신, 사용자 인터페이스를 통해 에이전트의 지침, 설정, 도구를 구성합니다.

1. Microsoft Foundry 확장 보기에서 **리소스** 섹션을 찾습니다.

1. **선언적 에이전트** 하위 섹션 옆에 있는 **+**(더하기) 아이콘을 선택하여 새 AI 에이전트를 만듭니다.

    ![Microsoft Foundry VS Code 확장에서 에이전트를 만드는 스크린샷](Media/vs-code-new-agent.png)

1. 메시지가 표시되면 에이전트 파일을 저장할 위치를 선택합니다.

1. **새 에이전트** 탭이 `.yaml` 구성 파일과 함께 "에이전트 기본 설정" 편집기에서 열립니다.

### 디자이너에서 에이전트 구성

1. 에이전트 기본 설정에서 다음 필드를 구성합니다.
...
![Foundry VS Code 확장에서 에이전트에 도구를 추가하는 스크린샷](Media/vs-code-agent-tools.png)
...
## Microsoft Foundry에 에이전트 배포

1. 에이전트 디자이너 보기의 왼쪽 아래 모서리에 있는 **Microsoft Foundry에서 에이전트 만들기** 단추를 선택합니다.

1. 배포가 완료될 때가지 기다립니다.

1. VS Code 탐색 모음에서 **리소스** 보기를 새로 고칩니다. 이제 배포된 에이전트가 **선언적 에이전트** 하위 섹션 아래에 표시됩니다.

## 플레이그라운드의 에이전트 테스트

1. **선언적 에이전트** 하위 섹션에서 배포된 에이전트를 마우스 오른쪽 단추로 클릭합니다.
...
이 연습에서는 Foundry VS Code 확장을 사용하여 MCP 서버 도구로 AI 에이전트를 만들었습니다. 에이전트는 모델 컨텍스트 프로토콜을 통해 외부 데이터 원본 및 API에 액세스할 수 있으므로 최신 정보를 제공하고 다양한 서비스와 상호 작용할 수 있습니다. 또한 플레이그라운드에서 에이전트를 테스트하고 프로그래밍 방식 상호 작용을 위한 샘플 코드를 생성하는 방법을 알아보았습니다.

## 정리

Foundry VS Code 확장 탐색을 마쳤으면 불필요한 Azure 비용이 발생하지 않도록 리소스를 정리해야 합니다.

### 에이전트 삭제

1. Foundry 포털의 탐색 메뉴에서 **에이전트**를 선택합니다.
