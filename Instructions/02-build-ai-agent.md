...
1. **Connect to the Agent client** 주석을 찾아 다음 코드를 추가하여 Azure AI 프로젝트에 연결합니다.
   ...
   with agent_client:
    ...
    코드는 현재 Azure 자격 증명을 사용하여 Foundry 프로젝트에 연결됩니다. 마지막 *with agent_client* 문은 클라이언트의 범위를 정의하는 코드 블록을 시작하여 블록 내의 코드가 완료되면 정리되도록 합니다.
...
1. **Check the run status for failures** 주석을 찾고 다음 코드를 추가하여 오류가 있는지 확인합니다.
