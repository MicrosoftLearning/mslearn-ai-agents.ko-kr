...
## Microsoft Foundry 프로젝트에 모델 배포

먼저 Foundry 프로젝트에 모델을 배포해 보겠습니다.
...
1. 왼쪽 탐색 창에서 **개요**를 선택하면 다음과 같은 프로젝트의 메인 페이지가 표시됩니다.
    ...
    ![Foundry 포털의 Azure AI 프로젝트 세부 정보 스크린샷](./Media/ai-foundry-project.png)
...
1. 코드 파일에서 **your_openai_endpoint** 자리 표시자를 프로젝트의 엔드포인트(Foundry 포털의 프로젝트 **개요** 페이지에서 복사)로 바꿉니다. **your_model_deployment** 자리 표시자를 GPT-4o 모델 배포에 할당한 이름으로 바꿉니다.
...
1. **채팅 클라이언트 만들기** 주석 아래에 다음 코드를 추가합니다.
    ...
    **AzureCliCredential** 개체를 사용하면 코드에서 Azure 계정에 인증할 수 있습니다. **AzureAIAgentClient** 개체는 .env 구성의 Foundry 프로젝트 설정을 자동으로 포함합니다.
