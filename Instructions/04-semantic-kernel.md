...
이 연습에서는 Azure AI 에이전트 서비스 및 의미 체계 커널을 사용하여 비용 청구 이메일을 만드는 AI 에이전트를 만듭니다.
...
1. 이 파일의 코드를 검토합니다. 여기에는 다음이 포함됩니다.
    ...
    - 비용 클레임에 대한 데이터를 정의하는 *main* 함수입니다(실제 애플리케이션에서는 파일로 제출될 가능성이 높음).
    - 에이전트를 만들고 사용할 코드를 추가해야 하는 **create_expense_claim** 함수
...
1. **Use the agent to generate an expense claim email** 주석을 찾아 다음 코드를 추가하여 에이전트를 실행할 스레드를 만든 다음 채팅 메시지를 사용하여 호출합니다.
