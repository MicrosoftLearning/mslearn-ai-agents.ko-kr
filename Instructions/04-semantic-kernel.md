...
1. 새 **EmailPlugin** 클래스 코드 위에 있는 **process_expenses_data** 함수에서 **구성 설정 받기** 주석을 찾아 다음 코드를 추가하여 구성 파일을 로드하고 **AzureAIAgentSettings** 개체(구성에서 Azure AI 에이전트 설정을 자동으로 포함함)를 만듭니다.
